    ...
    <div class="membercard_set">
      ...
      <el-form ref="form" :model="form" label-width="150px" :rules="rules">
        ...
        <el-form-item label="卡片背景色 :" prop="bgColor">
          <el-color-picker v-model="colorStyle.background" color-format='rgb' @change='colorchange'></el-color-picker>
        </el-form-item>
        <!--正面背景-->
        <el-form-item label="卡片背景图片 :" prop="bgUrl">
          <el-upload
          ...
          <span class="ticket_tip ">备注 : 5M以内，格式：png、jpg； 尺寸不小于1020*643px； 不得有圆角，不得拉伸变形;</span>
        </el-form-item>
        <!--logo-->
        <el-form-item label="LOGO图片 :" prop="logoUrl">
          ...
          <span class="ticket_tip">备注 : 1M以内，格式png、jpg;尺寸不小于500*500px的正方形;请优先使用商家LOGO;</span>
        </el-form-item>
        <el-form-item label="会员权益 :" prop="memo" >
          ...
          <span class="ticket_tip">提示 : 内容将展示在支付宝会员卡会员权益说明中,如有多条说明回车换行即可!</span>
        </el-form-item>
        ...
        <el-form-item label="用户信息获取 : ">
            ...
            v-model="personaldata">
                ...
                <!-- <el-checkbox v-for="item in personallist" :label="item" :key="item">{{item}}</el-checkbox> -->
            </el-checkbox-group>
        ...
        <el-form-item label="栏位设置 :">
          ...
          <div style="margin-bottom: 10px;" v-for="(item,index) in lwlist">
              <div>
                <span>({{index+3}}) </span><span><i style="color:#ff4949;">* </i>标题 : </span><el-input style="display:inline-block;width: 140px"  size="small" v-model="item.title"  placeholder="请填写标题"></el-input>&#X3000;
          ...
          <el-button class="margin-l-f" size="small" v-if="lwlist.length <8" type="text"  @click="addDomain">新增栏目</el-button>
          <!-- <div style="margin-bottom: 10px;" v-for="(item,index) in lwlist">
              <div>
          ...
          &#X3000; <el-button size="small" v-if="lwlist.length <5" type="text"  @click="addDomain">新增栏目</el-button> -->
        </el-form-item>
        ...
        <el-form-item>
            <el-button v-if=form.id class="bottom_button" size="small" :disabled="thisdisabled" type="primary" @click="setMembercard('form')">修 改
            </el-button>
            <el-button v-if=!form.id class="bottom_button" size="small" :disabled="thisdisabled" type="primary" @click="setMembercard('form')">提 交
            </el-button>
        </el-form-item>
      </el-form>
      ...
      <el-form label-width="150px">
        ...
        <div style="padding-left:80px;margin-top:20px;margin-bottom:10px;">
            <el-table
                class="inputwidth_t"
                :data="hy_tableData"
                style="max-width: 500px"
        ...
        <el-form-item>
          <el-button style="margin-bottom:40px;" :disabled="checksuredisabled" class="bottom_button" size="small" type="primary" @click="checksure">确 定</el-button>
        </el-form-item>
  ...
  export default {
    data(){
      return {
        ...
        checkedCities1:['积分'],
        cities: typeOptions,
        //8a99afc65caf86fc015caf9e751e000b
        shopid:{id:''},
        converRates:'',
        selectdata: [
        ...
        imageUrltobg: '',
        imageUrltologo: '',
        bgimgurl: require('../../assets/images/membercard.png'),
        logoimgurl: require('../../../src/assets/images/card_logo.png'),
        colorStyle: {
    ...
    methods: {
        ...
        up_Membercard(){
            ...
            this.form.hiddens = this.isshow1 + ',' + this.isshow2;
            ...
            if(this.personaldata.length == 0){
                ...
                // this.form.regFiled = 'BIRTHDAY,IDCARD,MOBILE';
                this.sumbit_regFiled = '';
            }
            ...
            this.$http.post(`${ipAddress}/front/card_template!saveCardTemplateByMerchant.action`,data)
            .then(res=>{
        ...
        getMembercard(){
            queryLogin();
            this.$http.post(`${ipAddress}/front/card_template!getCardTemplateByMerchant.action`)
            .then( res => {
                ...
                if (res.data.content.sendPoint) {
                    this.form.sendPoint = res.data.content.sendPoint.toString();
                }
                this.colorStyle.background = res.data.content.bgColor;
                this.logoimgurl = res.data.content.logoUrl;
                this.imageUrltologo = res.data.content.logoUrl;
                this.bgimgurl = res.data.content.bgUrl;
                this.imageUrltobg = res.data.content.bgUrl;
                if(res.data.content.fieldList == 'POINT'){
                    this.checkedCities1 = ['积分'];
                }
                if(res.data.content.fieldList == 'BALANCE'){
                    this.checkedCities1 = ['余额'];
                }
                if(res.data.content.fieldList == 'POINT,BALANCE'){
                    this.checkedCities1 = ['积分','余额'];
                }
                res.data.content.hiddens.charAt(0) == '0' ? this.isshow1 = 0 : this.isshow1 = 1;
                res.data.content.hiddens.charAt(1) == '0' ? this.isshow2 = 0 : this.isshow2 = 1;
                // res.data.content.hiddens.charAt(2) == '0' ? this.isshow3 = 0 : this.isshow3 = 1;
                if(res.data.content.regFiled){
                    this.getpersonaldata = res.data.content.regFiled.replace('IDCARD','身份证').replace('MOBILE','手机号').replace('BIRTHDAY','生日').replace('NAME','姓名');
                    this.personaldata = this.getpersonaldata.split(','); 
                }
                if(res.data.content.cardTemplateColumnList){
                    for(let i = res.data.content.cardTemplateColumnList.length-1;i>=0;i--){
                        if(res.data.content.cardTemplateColumnList[i].title == '积分兑券'){
                             res.data.content.cardTemplateColumnList.splice(i, 1);
                             continue;
                        }
                        if(res.data.content.cardTemplateColumnList[i].title == '积分换礼品'){
                             res.data.content.cardTemplateColumnList.splice(i, 1);
                             continue;
                        }
                    }
                    this.lwlist = res.data.content.cardTemplateColumnList;
                }
                if(res.data.content.cardTemplateLevelList){
                    this.gradetabledata = res.data.content.cardTemplateLevelList;
                }
            })
        },
        //获取会员积分设置
        memberPoints: function () {
            this.$http.post(`${ipAddress}/front/conver_rate!getRateByMerchant.action`)
            .then(res => {
                console.log(res.data);
                this.hy_tableData = res.data.content.converRateList;
                this.hy_enable = res.data.content.converRateList[0].enable;
            })
        },
        //验证会员积分设置
        checksure() {
        ...
        sure: function () {
            ...
            this.$http.post(`${ipAddress}/front/conver_rate!saveRateByMerchant.action`,data)
            .then(res => {
        ...
        beforeAvatarUpload(file) {
            this.$message('正在上传卡片背景图片!');
        },
        logobeforeAvatarUpload(file) {
            this.$message('正在上传LOGO图片!');
        },
    },
    created: function () {
...
<style>
    ...
    .inputwidth_t .el-input{
        width: 60px !important;
        margin-top: 4px;
    }
    .inputwidth_t_grade .el-input{
        width: 60px !important;
    }
    .inputwidth_t_grade .cell{
        margin-top: 10px;
    }
    .inputwidth_t_grade span{
        line-height: 30px;
    }
    .inputwidth_t img{
        width: 12px;

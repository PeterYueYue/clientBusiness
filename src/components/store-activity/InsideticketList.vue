...
<template>
  <div class="busarea_active_main">
    <div class="tab_header busarea-active-title">
      ...
      <!--<option value=""  v-for="selectBusareas in selectBusarea">{{selectBusareas.value}}</option>-->
      <!--</select>-->
...
//      列表
      getList: function (data) {
        queryLogin();
        this.$http.post(`${ipAddress}/front/inner_voucher!getInerVoucherPager.action`, data)
          .then(res => {
            this.disabled = false;
...
//      搜索
      query: function () {
        queryLogin();
        if (this.valuez) {
          this.datas.isMarketable = this.valuez;
        }
        if (this.valuem) {
          this.datas.shopInfo = this.valuem;
        }
        let message = this.qs.stringify(this.datas);
        this.disabled = true;
        this.getList(message);

    ...
    <div class="pop-busarea-body">
      ...
      <div class="pop-busarea-title">
        <div class="pop-busarea-top">
          ...
          <div class="pop-div">
            <div class="pop-avatar danping_imgbg">
              <span></span>
            </div>
            <div class="pop-word pop-m-word">
              <span>{{message.voucher_worth_value.toFixed(2)}}</span><span> 元&#160;代金券</span>
              <p class="pop-download"><span @click="QRcode(message.url)" v-if="message.url !=''">二维码</span><span
                @click="dowloadImg(message.url)" v-if="message.url !=''">下载</span></p>
            </div>
          </div>
        </div>
      </div>
        ...
        <div class="pop-busarea-tops" v-show="preview">
               <span>
                <div class="pop-busarea-p">
                  ...
                  <p><span>活动有效期 : </span><span> {{message.start_time}} - {{message.end_time}}</span></p>
                  <p><span>活动类型 : </span><span> {{message.promo_type}}</span></p>
                  <p><span>适用门店 : </span><span> {{message.shopAmount}} 家门店</span></p>
                  <p><span>操作 : </span>
                    <router-link tag="span" class="pop-busarea-span" :to="{path:'viewdetails',query:{id:message.id}}">
                         点击设置
                    </router-link>
                  </p>
                </div>
          ...
          <span>
                <button v-if="message.url ==''"  @click="waterTo(message.activityId)">查看流水</button>
              </span>
        </div>

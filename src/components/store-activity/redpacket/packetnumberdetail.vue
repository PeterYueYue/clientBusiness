  ...
  <div class="main">
    <div class="zj-mainss">
      ...
      <div class="zj-shape">
        ...
        <p v-if="form.radio == 1">每个用户领取的红包金额为活动预算金额除以红包发送数量得到的平均金额值得0.5-1.5倍</p>
        <!-- <p> -->
      ...
      <span style="margin-left: 640px;font-size: 12px" v-if="type== 3">每人{{date}}领取{{message.pickLimitNum}}次</span>
      ...
      <span style="margin-left: 640px;font-size: 12px" v-if="message.isPickLimit==1">{{message.pickTimeLimit}}</span>
      <span class="zj-foot">
        ...
        this.$http.post(`${ipAddress}/front/ali_cash_activity!getAliCashActivityById.action`, data)
          .then(res => {
            if (res.data.errorCode == 10000) {
              ...
              if (res.data.content.prizeType == 'random') {
                this.form.radio = '1';
              } else {
                this.form.radio = '2';
              }
              this.number = res.data.content.shopInfoList.length;
//              活动类型
              if (res.data.content.cashType == 0) {
//                消费返还
                this.type = 0
              } else if (res.data.content.cashType == 1) {
//                指定单品
                this.type = 1
              } else if (res.data.content.cashType == 2) {
//                新会员
                this.type = 2
              } else if (res.data.content.cashType == 3) {
//                消费集点送
                this.type = 3
              } else if (res.data.content.cashType == 4) {
//                新客奖励
                this.type = 4
              } else if (res.data.content.cashType == 5) {
//                回归礼
                this.type = 5
              }
              if (res.data.content.pickLimitType == "D") {
                this.date = '日'
              } else if (res.data.content.pickLimitType == "W") {
                this.date = '周'
              } else if (res.data.content.pickLimitType == "M") {
                this.date = '月'
              }
            }
          })
